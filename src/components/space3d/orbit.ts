 ...
 *
 * Считается он по радиусу сцены, а тот — корень из настоящего, поэтому
 * и отношение периодов выходит корнем из настоящего: Меркурий успевает
 * двадцать шесть оборотов, пока Нептун делает один, вместо шестисот
 * восьмидесяти. Это ровно тот же обмен, что и с расстояниями, и сделан
 * он ради того же — чтобы за внешними планетами было видно движение,
 * а не застывшая картинка.
 */
